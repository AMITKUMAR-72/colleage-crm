        ...
        <div className="space-y-6 mb-8">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {stats.map((stat, idx) => (
                    <div 
                        key={idx} 
                        className="p-6 rounded-3xl border border-slate-200/60 bg-white/50 backdrop-blur-md flex flex-col justify-between h-32 hover:border-slate-300 transition-colors shadow-sm"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white/50 backdrop-blur-md p-8 rounded-3xl border border-slate-200/60 shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="font-black text-slate-900 tracking-tight uppercase text-xs">Lead Quality Distribution</h3>
                    </div>
                ...
                <div className="bg-white/50 backdrop-blur-md p-8 rounded-3xl border border-slate-200/60 shadow-sm flex flex-col">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="font-black text-slate-900 tracking-tight uppercase text-xs">Daily Performance</h3>
                        <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-3 py-1 rounded-lg uppercase tracking-widest border border-emerald-100">On Track</span>
                    </div>
                    <div className="flex-1 flex items-end gap-3 h-32 px-2">
                        {[40, 70, 45, 90, 65, 80, 55].map((h, i) => (
                            <div key={i} className="flex-1 group relative">
                                <div 
                                    className="w-full bg-[#4d0101] rounded-t-md opacity-30 group-hover:opacity-80 transition-opacity" 
                                    style={{ height: `${h}%` }} 
                                />
