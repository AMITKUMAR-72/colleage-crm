...
function MultiAssignButton({ leadIds, lead, title, onAssigned, className, isBulk = false }: { 
    leadIds: number[]; 
    ...
    const handleOpen = async (e: React.MouseEvent) => {
        ...
        if (nowOpen && counselors.length === 0) {
            ...
            try {
                ...
                const isCourseNull = lead ? (!lead.course || (typeof lead.course === 'object' && !(lead.course as any).course)) : false;
                if (isCourseNull) {
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-right" onClick={e => e.stopPropagation()}>
                                            <div className="flex items-center justify-end gap-2 text-right">
                                                ...
                                                <MultiAssignButton leadIds={[lead.id]} lead={lead} onAssigned={() => { fetchLeads(); setSelectedIds(new Set()); }} />
                                            </div>
                                        </td>
                                    </tr>
                                ))}
