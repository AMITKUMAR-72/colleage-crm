                ...
                <div className="bg-white border border-slate-200 rounded-[2rem] overflow-hidden shadow-sm">
                    <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                        <div>
                            ...
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Real-time system-wide monitoring</p>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="px-3 py-1 bg-white border border-slate-200 rounded-full text-[10px] font-black text-indigo-600">
                                {allLeaves.length} ENTRIES
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="p-6">
                        {allLeavesLoading ? (
                            <div className="py-12 text-center">
                                ...
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Loading Global Records…</p>
                            </div>
                        ) : (
                            <div className="flex flex-col gap-2">
                                {allLeaves.slice((currentPage - 1) * leavesPerPage, currentPage * leavesPerPage).map((l, i) => (
                                    <div key={l.id || i} className="p-4 bg-white border border-slate-100 rounded-xl flex items-center justify-between hover:bg-slate-50 transition-all">
                                        <div className="flex items-center gap-6">
                                            <div className={`w-2 h-2 rounded-full ${
                                                l.status === 'APPROVED' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' :
                                                l.status === 'REJECTED' ? 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.5)]' :
                                                'bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]'
                                            }`} />
                                            <p className="text-sm font-bold text-slate-700 uppercase tracking-tight truncate max-w-[200px]">{l.reason}</p>
                                        </div>
                                        <div className="flex items-center gap-8">
                                            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-100 px-3 py-1 rounded-md">
                        ...
                        {allLeaves.length > leavesPerPage && (
                            <div className="mt-8 flex items-center justify-center gap-4 border-t border-slate-100 pt-6">
                                <button
                                    ...
                                    className="px-6 py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase text-slate-500 hover:bg-slate-50 disabled:opacity-30 transition-all"
                                >
                                    Previous
                                </button>
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 grid grid-cols-1 md:grid-cols-3 items-center gap-4">
                                                ...
                                                <div className="hidden sm:flex items-center gap-3">
                                                    ...
                                                    <div className="text-[10px] font-bold text-slate-500 uppercase tracking-widest truncate">{c.department || 'Counselor'}</div>
                                                </div>
                                            </div>
                            ...
                            <div className="bg-slate-50 border-b border-slate-200 p-6 flex flex-col sm:flex-row items-center justify-between gap-6">
                                ...
                                <div className="flex flex-col sm:items-end text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                                    ...
                                    <div className="flex items-center gap-2 mt-1 px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-md border border-indigo-100">
                                        ...
                                        <span>{selectedCounselor?.department || 'Department'}</span>
                                    </div>
                                </div>
                            </div>
